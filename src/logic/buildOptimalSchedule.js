  ...
  switch (vk) {
    ...
    case 'PCV': {
      if (am >= 24 && isHRPCV) {
        ...
        // (buildOptimalSchedule is pediatric-only — am>=228 returns [] before
        // seriesDoses() is ever called — so no separate adult branch exists here.)
    ...
    case 'PPSV23': {
      ...
      const completingPCV =
        (hist.PCV || []).some(x => x.given && (x.brand?.startsWith('Prevnar 20') || x.brand?.startsWith('Capvaxive'))) ||
        Object.entries(fcBrands).some(([k, b]) => k.endsWith('_PCV') && (b.startsWith('Prevnar 20') || b.startsWith('Capvaxive')));
      if (completingPCV) return null;
      const ppsvCount = (hist.PPSV23 || []).filter(d => d.given).length;
