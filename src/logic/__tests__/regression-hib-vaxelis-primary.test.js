...
describe('Pure 3-dose Vaxelis primary series — series complete after D3, no booster expected', () => {
  ...
  const hist = {
    Hib: [
      ...
      dose('2024-07-01', 'Vaxelis'),    // D3 ~6m
    ],
  };

  it('getTotalDoses("Hib") returns 3 when all Vaxelis', () => {
    // Mock rec object — Vaxelis OMP, series is 3 total
  ...
  it('genRecs at 12m with 3 Vaxelis doses does NOT emit a Hib booster rec', () => {
    // 12m patient, 3 Vaxelis doses already given — Vaxelis series complete, no booster
    const recs = genRecs(12, hist, [], dob, {});
    const hibRecs = recs.filter(r => r.vk === 'Hib');
    expect(hibRecs).toHaveLength(0);
  });
});
